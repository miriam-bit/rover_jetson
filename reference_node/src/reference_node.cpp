...
static inline SmoothedAnalog_StatusTypeDef driveMotors( const Cartesian2D& analog, float max_output_forward,float max_output_backward, int16_t& l_wheel, int16_t& r_wheel) 
{
    ...
    if ((y > 0.0f) && (x >= 0.0f)) {
        ...
        temp_r = temp_l - (x / max_input_forward * max_output_forward * max_turn_reg);
        ...
        temp_r = temp_l + (x / max_input_forward * max_output_backward * max_turn_reg);
    } else if ((y < 0.0f) && (x < 0.0f)) {
        temp_l = (y / max_input_backward * max_output_backward) -
    ...
    temp_l = clamp(std::round(temp_l), -max_output_backward, max_output_backward);
    temp_r = clamp(std::round(temp_r), -max_output_backward, max_output_backward);

    l_wheel = static_cast<int16_t>(temp_l);
    r_wheel = static_cast<int16_t>(temp_r);
...
private:
    ...
    * -------------------------------------------------------------------------- */
    ...
    void joyCallback(const sensor_msgs::msg::Joy::SharedPtr msg) {
        ...
        static const float deadzone_y = 0.01;
